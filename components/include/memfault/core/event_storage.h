...
//! to appropriately size the full state storage buffer.
  ...
  #define MEMFAULT_EVENT_STORAGE_STATE_SIZE_BYTES 44UL
#elif (INTPTR_MAX == 0x7fffffffffffffff)
  #define MEMFAULT_EVENT_STORAGE_STATE_SIZE_BYTES 80UL
#endif
...
//! system restarts, and is called from memfault_events_storage_boot. It's
//! weakly defined to do nothing. A platform can implement this function to
//! restore the event storage state after a deep sleep or power loss.
//!
//! Must be implemented if MEMFAULT_EVENT_STORAGE_RESTORE_STATE is enabled.
//!
//! @param state The state to restore- should match the state returned
//!   from memfault_event_storage_get_state. This function assumes the state
//!   is valid (uncorrupted).
//!
