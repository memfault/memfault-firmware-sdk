...
//! 3. call memfault_platform_coredump_storage_boot() from your memfault_platform_boot()
//! implementation
...
void memfault_platform_coredump_storage_boot(void) {
  ...
  const bool exists =
    ad_nvms_get_partition_info(MEMFAULT_PLATFORM_COREDUMP_STORAGE_PARTITION, &partition_info);
  if (!exists) {
    MEMFAULT_LOG_ERROR(
      "Could not locate partition for coredump storage, has ad_nvms_init() been called?");
    return;
  }

  // ad_nvms_open() should not fail if above check succeeds.
  s_qspi_coredump_partition_info.partition.handle =
    ad_nvms_open(MEMFAULT_PLATFORM_COREDUMP_STORAGE_PARTITION);
  const size_t partition_size = ad_nvms_get_size(s_qspi_coredump_partition_info.partition.handle);
  s_qspi_coredump_partition_info.partition.size =
    MEMFAULT_MIN(partition_size, MEMFAULT_PLATFORM_COREDUMP_STORAGE_MAX_SIZE_BYTES);
...
bool memfault_platform_coredump_storage_read(uint32_t offset, void *data, size_t data_len) {
