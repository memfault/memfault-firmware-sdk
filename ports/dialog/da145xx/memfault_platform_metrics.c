...
bool memfault_platform_metrics_timer_boot(uint32_t period_sec,
  ...
  if (app_easy_timer(MS_TO_TIMERUNITS(period_sec * 1000), callback) == EASY_TIMER_INVALID_TIMER) {
    rc = false;
  }
  return rc;
}
...
uint64_t memfault_platform_get_time_since_boot_ms(void) {
  if (GetBits16(CLK_RADIO_REG, BLE_ENABLE) == 0) {
    // The BLE core timer is not running so we just return 0. Generally, the core should always be
    // running
    return 0;
  }
