...
bool memfault_platform_coredump_storage_read(uint32_t offset, void *data, size_t read_len) {
  ...
  // Special case: if the first word is 0, the coredump has been cleared and
  // reads should return all zeros.
  const uint32_t first_word = *(const uint32_t *)(MEMFAULT_COREDUMP_PARTITION_ADDRESS);
  if (first_word == 0) {
...
//! coredump (the SDK treats a zero first-word as "no coredump present").
