...
static uint32_t s_software_watchdog_timeout_ms = MEMFAULT_WATCHDOG_SW_TIMEOUT_SECS * 1000;
...
int memfault_software_watchdog_enable(void) {
  prv_start_or_reset(s_software_watchdog_timeout_ms);
  return 0;
}
...
int memfault_software_watchdog_update_timeout(uint32_t timeout_ms) {
  s_software_watchdog_timeout_ms = timeout_ms;
  prv_start_or_reset(s_software_watchdog_timeout_ms);
  return 0;
}
