...
//! User-settable group access callback and argument
static struct {
  memfault_mgmt_access_cb_t cb;
  void *user_arg;
} s_memfault_mgmt_access = {
  .cb = NULL,
  .user_arg = NULL,
};

static bool prv_memfault_mgmt_is_access_allowed(void) {
  if (s_memfault_mgmt_access.cb != NULL) {
    return s_memfault_mgmt_access.cb(s_memfault_mgmt_access.user_arg);
  }
  return true;
}

void memfault_mcumgr_set_access_callback(memfault_mgmt_access_cb_t cb, void *user_arg) {
  s_memfault_mgmt_access.cb = cb;
  s_memfault_mgmt_access.user_arg = user_arg;
}
...
static int memfault_mgmt_device_info(struct smp_streamer *ctxt) {
  if (!prv_memfault_mgmt_is_access_allowed()) {
  ...
  // Store ternary results in temporary variables to avoid redundant evaluation
  const char *device_serial = device_info.device_serial ? device_info.device_serial : "";
  const char *hardware_version = device_info.hardware_version ? device_info.hardware_version : "";
  const char *software_type = device_info.software_type ? device_info.software_type : "";
  const char *software_version = device_info.software_version ? device_info.software_version : "";

  // Encode the response as CBOR
  bool ok = zcbor_tstr_put_lit(zse, "device_serial") &&
            zcbor_tstr_put_term(zse, device_serial, strlen(device_serial)) &&
            zcbor_tstr_put_lit(zse, "hardware_version") &&
            zcbor_tstr_put_term(zse, hardware_version, strlen(hardware_version)) &&
            zcbor_tstr_put_lit(zse, "software_type") &&
            zcbor_tstr_put_term(zse, software_type, strlen(software_type)) &&
            zcbor_tstr_put_lit(zse, "current_version") &&
            zcbor_tstr_put_term(zse, software_version, strlen(software_version));
