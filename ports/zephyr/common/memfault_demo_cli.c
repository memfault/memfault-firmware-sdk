...
static int prv_check_and_fetch_ota_payload_cmd(const struct shell *shell, size_t argc,
                                               char **argv) {
#if defined(CONFIG_MEMFAULT_ZEPHYR_FOTA)
  return memfault_zephyr_fota_start();
#else
  shell_print(shell, "CONFIG_MEMFAULT_ZEPHYR_FOTA not enabled");
#endif
  return 0;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
  ...
  SHELL_CMD(test, &sub_memfault_crash_cmds,
            ...
            NULL),
#if defined(CONFIG_MEMFAULT_PROJECT_KEY_SETTINGS)
  SHELL_CMD_ARG(set_project_key, NULL, "get or set Memfault project key", prv_set_project_key, 1,
                1),
#endif
  SHELL_SUBCMD_SET_END /* Array terminated. */
);
