...
bool memfault_platform_coredump_storage_write(uint32_t offset, const void *data,
                                              size_t data_len) {
  /*if
  (!prv_op_within_flash_bounds(offset, data_len)) {
    return false;
  }
...
bool memfault_platform_coredump_storage_read(uint32_t offset, void *data,
  ...
  ret = mem_flash_area_open(COREDUMP_STORAGE_FLASH, &fa);
  if(OK != ret) {
    return false;
  }

  ret = mem_flash_area_read(fa, offset, data, read_len);
  if(OK != ret) {
    return false;
  }

  mem_flash_area_close(fa);
  if(OK != ret) {
    return false;
  }
...
bool memfault_platform_coredump_storage_erase(uint32_t offset, size_t erase_size) {
  ...
  ret = mem_flash_area_erase(fa, offset, erase_size);
  if(OK != ret) {
    return false;
  }
