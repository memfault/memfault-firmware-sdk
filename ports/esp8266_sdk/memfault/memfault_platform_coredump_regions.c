...
const sRamRegions s_valid_8266_ram_regions[] = { { .start_addr = 0x3FFE8000, .length = 0x18000 },
                                                 { .start_addr = 0x40100000,
                                                   .length = CONFIG_SOC_IRAM_SIZE } };
...
const sMfltCoredumpRegion *memfault_platform_coredump_get_regions(
  ...
  s_coredump_regions[region_idx++] =
    MEMFAULT_COREDUMP_MEMORY_REGION_INIT(_bss_start, (uint32_t)(_bss_end) - (uint32_t)_bss_start);
  ...
  s_coredump_regions[region_idx++] = MEMFAULT_COREDUMP_MEMORY_REGION_INIT(_bss_end, bss_size);
  ...
  s_coredump_regions[region_idx++] = MEMFAULT_COREDUMP_MEMORY_REGION_INIT(_iram_end, iram_size);
    #endif
