...
static void prv_encode_with_fill_interval(sMemfaultRleResultCtx *result_ctx, size_t total_size,
                                          size_t fill_call_size) {
...
TEST(MemfaultRle, Test_QuickTransitions) {
  const uint8_t pattern[] = { 0x1, 0x2, 0x2, 0x3, 0x4, 0x4, 0x5, 0x6, 0x6, 0x7, 0x8, 0x8 };

  const uint8_t expected[] = {
    0x17, 0x01, 0x02, 0x02, 0x03, 0x04, 0x04, 0x05, 0x06, 0x06, 0x07, 0x08, 0x08,
  };
...
TEST(MemfaultRle, Test_2and3RunSequences) {
  const uint8_t pattern[] = {
    0x1, 0x2, 0x2,  // 2-byte pattern too short, gets included in current non-repeating sequence
    0x3, 0x4, 0x4, 0x4,  // 3-byte pattern should start new repeating sequence
    0x5, 0x6, 0x6, 0x6, 0x7, 0x7, 0x8, 0x8
  };
...
TEST(MemfaultRle, Test_VariousEncoderSizes) {
  uint8_t pattern[400] = {
    0x1, 0x2, 0x3, 0x3, 0x3, 0x3, 0x5, 0x6, 0x7,
  };
  ...
  for (size_t i = 1; i < sizeof(pattern); i++) {
    ...
    const uint8_t expected[] = { 0x03, 0x01, 0x02, 0x08, 0x03, 0x05,
                                 0x05, 0x06, 0x07, 0x8E, 0x06, 0x00 };
