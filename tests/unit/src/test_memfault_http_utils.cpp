...
TEST(MfltHttpClientUtils, Test_MfltHttpClientGetOtaPayload) {
  ...
  bool success = memfault_http_get_ota_payload(prv_http_write_cb, &ctx, url, strlen(url));
  CHECK(success);

  const char *expected_string = "GET /path/to/ota/payload/yay HTTP/1.1\r\n"
...
TEST(MfltHttpClientUtils, Test_MfltHttpClientGetOtaPayloadNoPath) {
  ...
  const char *expected_string = "GET / HTTP/1.1\r\n"
                                "Host:example.ota.payload.com\r\n"
                                "User-Agent:MemfaultSDK/" MEMFAULT_SDK_VERSION_STR "\r\n"
                                "\r\n";
...
TEST(MfltHttpClientUtils, Test_MfltResponseParser202) {
  const char *rsp = "HTTP/1.1 202 Accepted\r\n"
...
TEST(MfltHttpClientUtils, Test_VeryLongBody) {
  const char *rsp_hdr = "HTTP/1.1 202 Accepted\r\n"
                        "Content-Type: text/plain; charset=utf-8\r\n"
                        "Content-Length: 1024\r\n"
                        "Date: Wed, 27 Nov 2019 22:52:57 GMT\r\n"
                        "Connection: keep-alive\r\n"
...
static void prv_expect_parse_failure(const void *response, size_t response_len,
  ...
  sMemfaultHttpResponseContext ctx = {};
  bool done = memfault_http_parse_response(&ctx, response, response_len);
  CHECK(done);
  LONGS_EQUAL(expected_parse_error, ctx.parse_error);

  // should also fail if we only parse the header
  memset(&ctx, 0x0, sizeof(ctx));
  done = memfault_http_parse_response_header(&ctx, response, response_len);
  CHECK(done);
  LONGS_EQUAL(expected_parse_error, ctx.parse_error);
}
...
TEST(MfltHttpClientUtils, Test_HttpResponseUnexpectedlyLong) {
  ...
  const char *rsp =
    ...
    "Content-Type: text/plain; charset=utf-8\r\n"
    "ReallyLongHeader:ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789;"
...
TEST(MfltHttpClientUtils, Test_HttpResponseUnexpectedlyLongFirstLine) {
  ...
  prv_expect_parse_failure(rsp, strlen(rsp), MfltHttpParseStatus_HeaderTooLongError);
}

TEST(MfltHttpClientUtils, Test_HeaderBeforeStatus) {
  const char *rsp = "Content-Type: text/plain; charset=utf-8\r\n"
                    "HTTP/1.1 202 Accepted\r\n"
                    "Content-Length: 1024\r\n"
                    "Date: Wed, 27 Nov 2019 22:52:57 GMT\r\n"
                    "Connection: keep-alive\r\n"
                    "\r\n";
  prv_expect_parse_failure(rsp, strlen(rsp), MfltHttpParseStatus_ParseStatusLineError);
}
...
TEST(MfltHttpClientUtils, Test_MfltResponseShort) {
  ...
  prv_expect_parse_failure(rsp_short, strlen(rsp_short), MfltHttpParseStatus_ParseStatusLineError);
...
TEST(MfltHttpClientUtils, Test_MfltResponseNoColonSeparator) {
  const static char *rsp = "HTTP/1.1 202 Accepted\r\n"
                           "Content-Length&10\r\n\r\n";
  prv_expect_parse_failure(rsp, strlen(rsp), MfltHttpParseStatus_ParseHeaderError);
}

static void prv_check_result(const char *uri, const char *host, const char *path,
                             bool expect_success, int expected_port) {
  ...
  LONGS_EQUAL(is_http_scheme ? kMemfaultUriScheme_Http : kMemfaultUriScheme_Https, uri_info.scheme);
  LONGS_EQUAL(expected_port, uri_info.port);
}

static void prv_uri_parse_check(const char *scheme, const char *host, const char *path,
                                bool expect_success) {
  char uri[256];
...
TEST(MfltHttpClientUtils, Test_MfltParseHttpAndHttpsUris) {
  ...
  for (size_t i = 0; i < MEMFAULT_ARRAY_SIZE(valid_prefixes); i++) {
    ...
    const char *paths[] = { "/", "/a/b/c/d?param1=1&param2=2", "" };
    ...
    for (size_t j = 0; j < MEMFAULT_ARRAY_SIZE(paths); j++) {
      ...
      prv_uri_parse_check(p, "[2001:0db8:85a3:0000:0000:8a2e:0370:7334]", path, expect_success);
      prv_uri_with_port_parse_check(p, "[2001:0db8:85a3:0000:0000:8a2e:0370:7334]", 832, path,
                                    expect_success);
    }
  }
}
