...
bool memfault_platform_device_info_boot(void) {
  // NOTE: This API will use an RTOS lock, so we cannot run this at the time of capturing the
  // coredump! Use the WiFi station MAC address as unique device id:
  return (WWD_SUCCESS ==
          wwd_wifi_get_mac_address(&s_memfault_platform_device_info_mac, WWD_STA_INTERFACE));
}
...
static void prv_get_device_serial(char *buf, size_t buf_len) {
  ...
  for (size_t i = 0; i < sizeof(s_memfault_platform_device_info_mac); i++) {
    ...
    int bytes_written = snprintf(&buf[curr_idx], space_left, "%02X",
                                 (int)s_memfault_platform_device_info_mac.octet[i]);
