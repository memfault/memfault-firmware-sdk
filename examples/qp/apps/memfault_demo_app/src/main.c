...
//   NOTE: After applying the ports/qp/qassert.h.patch and ports/qp/qf_pkg.h.patch files from the
//   Memfault Firmware SDK, the Q_onAssert function will no longer be necessary. Instead, Memfault's
//   assertion handling will be used. To ensure that no code relies on Q_onAssert, we recommend
//   removing the Q_onAssert function and replace any usage of it with the MEMFAULT_ASSERT macro
//   from memfault/panics/assert.h.
// }
