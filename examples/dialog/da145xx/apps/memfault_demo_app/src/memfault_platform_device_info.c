...
static void prv_get_device_serial(char *buf, size_t buf_len) {
  ...
  if (nvds_get(NVDS_TAG_BD_ADDRESS, &tag_len, (uint8_t *)&dev_bdaddr) == NVDS_OK) {
    (void)snprintf(buf, buf_len, "%02X%02X%02X%02X%02X%02X", dev_bdaddr.addr[5], dev_bdaddr.addr[4],
                   dev_bdaddr.addr[3], dev_bdaddr.addr[2], dev_bdaddr.addr[1], dev_bdaddr.addr[0]);
  }
}
