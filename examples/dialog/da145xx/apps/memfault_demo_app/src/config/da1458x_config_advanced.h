...
#include "da1458x_stack_config.h"
  ...
  /*                                                                                                              */
  /* NOTE: Disable CFG_XTAL16M_ADAPTIVE_SETTLING flag when RCX is chosen as the LP clock either from
   * the OTP      */
  ...
  /****************************************************************************************************************/
  /* If defined the application uses a hardcoded value for XTAL16M trimming. Should be disabled for
   * devices       */
  /* where XTAL16M is calibrated and trim value is stored in OTP. */
  /* Important note. The hardcoded value is the average value of the trimming values giving the
   * optimal results   */
  ...
  /****************************************************************************************************************/
  /* Enables True Random Number Generator. A true random number, generated at system initialization,
   * is used to   */
  /* seed any random number generator (C standard library, ChaCha20, etc.). The following supported
   * options       */
  ...
  /****************************************************************************************************************/
  /* Uses ChaCha20 random number generator instead of the C standard library random number
   * generator.             */
  /****************************************************************************************************************/
  ...
  /* - CFG_NVDS_TAG_BLE_CRA_TIMER_DUR     Channel Reassessment Timer duration (Multiple of CA timer
   * duration)     */
   ...
   */
  /* Application must be executed in Keil debugger environment and "da14585_586.lib" must be
   * replaced with        */
  /* "da14585_586_with_heap_logging.lib" in project structure under sdk_arch. Developer must stop
   * execution       */
  /* and type disp_heaplog() in debugger's command window. Heap memory statistics will be displayed
   * on window     */
  /****************************************************************************************************************/
  ...
  /****************************************************************************************************************/
  /* Maximum supported TX data packet length (supportedMaxTxOctets value, as defined in 4.2
   * Specification).       */
  /* Range: 27 - 251 octets. */
  /* NOTE 1: Even number of octets are not supported. A selected even number will be automatically
   * converted to   */
  /*         the next odd one. */
  /* NOTE 2: The supportedMaxTxTime value is automatically calculated by the ROM code, according to
   * the following */
  /*         equation: */
  ...
  /****************************************************************************************************************/
  /* Maximum supported RX data packet length (supportedMaxRxOctets value, as defined in 4.2
   * Specification).       */
  /* Range: 27 - 251 octets. */
  /* NOTE 1: Even number of octets are not supported. A selected even number will be automatically
   * converted to   */
  /*         the next odd one. */
  /* NOTE 2: The supportedMaxRxTime value is automatically calculated by the ROM code, according to
   * the following */
  /*         equation: */
  ...
  /*     - If the flag is defined, the extra devices are considered to be in the list and will not
   * be reported.   */
  /****************************************************************************************************************/
  ...
  /****************************************************************************************************************/
  /* Maximum retention memory in bytes. The base address of the retention data is calculated from
   * the selected    */
  /* size. */
  /****************************************************************************************************************/
  ...
  /****************************************************************************************************************/
  /* The Keil scatter file may be provided by the user. If the user provides his own scatter file,
   * the system has */
  /* to be aware which RAM blocks has to retain. The 4th RAM block is always retained, since it
   * contains the ROM  */
  /* data. */
  ...
  /*                                                                                                              */
  /* If the CFG_CUSTOM_SCATTER_FILE flag is undefined, the system knows which blocks to retain based
   * on the       */
  ...
  /* - CFG_HIGH_TEMPERATURE:         Device is configured to operate at high temperature range (-40C
   * to +105C).   */
  /* - CFG_AMB_TEMPERATURE:          Device is configured to operate at ambient temperature range
   * (-40C to +40C). */
  /* - CFG_MID_TEMPERATURE:          Device is configured to operate at mid temperature range (-40C
   * to +60C).     */
  /* - CFG_EXT_TEMPERATURE:          Device is configured to operate at ext temperature range (-40C
   * to +85C).     */
  /* NOTE 1: High temperature support is not compatible with power optimizations. User shall
   * undefine the         */
  /*         CFG_POWER_OPTIMIZATIONS flag, if device is to support the high temperature range
   * feature.            */
  /****************************************************************************************************************/
  ...
  /* NOTE: The XTAL16M adaptive settling algorithm works only with XTAL32K and not with RCX, as the
   * LP clock.     */
  /****************************************************************************************************************/
  ...
  /* Enables the logging of heap memories usage. The feature can be used in development/debug mode.
   */
  /* Application must be executed in Keil debugger environment and "da14531.lib" must be replaced
   * with            */
  /* "da14531_with_heap_logging.lib" in project structure under sdk_arch. Developer must stop
   * execution           */
  /* and type disp_heaplog() in debugger's command window. Heap memory statistics will be displayed
   * on window     */
  /****************************************************************************************************************/
  ...
  // 1. MEMFAULT_REBOOT_TRACKING_REGION_SIZE (64 bytes) - used to hold info about resets taking
  // place
  // 2. MEMFAULT_PLATFORM_COREDUMP_STORAGE_RAM_SIZE if and only if
  // MEMFAULT_PLATFORM_COREDUMP_STORAGE_USE_RAM=1
  ...
  /* The Keil scatter file may be provided by the user. If the user provides his own scatter file,
   * the system has */
  /* to be aware which RAM blocks has to retain. The 3rd RAM block is always retained, since it
   * contains the ROM  */
  /* data. */
  /*     - CFG_RETAIN_RAM_1_BLOCK: if defined, the 1st RAM block must be retained. */
  /*     - CFG_RETAIN_RAM_2_BLOCK: if defined, the 2nd RAM block must be retained. */
  ...
  /****************************************************************************************************************/
  /* Disable quadrature decoder on start up. The quadrature decoder is by default enabled on system
   * power up and  */
